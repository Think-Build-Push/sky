...
class _co__mem_ctlr extends _ctlr
{
	...
	public function get__co__mems() : array|bool
	{
		$mems = $this->obj->get_co__mems();
		if( FALSE === $mems )
		{
	...
	public function save() : bool
	{
		if( $_POST['_mem_email'] )
		{
			...
			{
				$this->fail( 'could_not_check_login_for_co__mem_save' );
				return FALSE;
			}

			if( $login_exists && $login_exists != $_POST['_co__mem_id'] )
			{
		...
		$co_mem = [];
		if( $_POST['_co__mem_id'] )
		{
			$co_mem = $this->fetch( $_POST['_co__mem_id'] );
		}
		else
		{
			$_mem = new _mem();
